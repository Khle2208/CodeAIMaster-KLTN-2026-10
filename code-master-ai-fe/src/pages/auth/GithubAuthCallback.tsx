...
export default function GithubAuthCallback() {
  ...
  // State để lưu tin nhắn lỗi nếu có
  ...
  useEffect(() => {
    ...
    if (!userString) {
      setErrorMessage("Không tìm thấy thông tin đăng nhập từ GitHub.");
      return;
    }
    ...
    const init = async () => {
      try {
        ...
        // } else {
          setErrorMessage("Có lỗi xảy ra trong quá trình xác thực với GitHub. Vui lòng thử lại.");
        // }
      }
    };
  ...
  // --- GIAO DIỆN ĐANG XỬ LÝ (MẶC ĐỊNH) ---
  return (
    <AuthLayout>
        <div className="text-center">
            <LoadingOutlined className="text-5xl text-brand-600 animate-spin mb-4" />
            <p className="text-brand-600 font-medium text-lg">Đang xử lý đăng nhập GitHub...</p>
