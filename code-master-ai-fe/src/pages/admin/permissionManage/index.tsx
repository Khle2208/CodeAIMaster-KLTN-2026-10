...
const RoleManage: React.FC = () => {
  ...
  const hasPermissionToEdit =
    userInfo?.permissions?.includes("permissions_view"); // Giả sử "permissions_view" là quyền được phép chỉnh sửa phân quyền
  ...
  const columns = [
    {
      ...
      width: "30%",
      render: (text: string) => (
        <strong className="text-brand-700">{text}</strong>
      ),
    },
    ...actions.map((action) => ({
      title: action.label,
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-sm text-brand-400">
            Thiết lập quyền hạn chi tiết cho từng nhóm tài khoản.
          </p>
        </div>
