...
import { NavLink, Outlet, useNavigate, useLocation, Navigate } from "react-router-dom"; // 🚨 Thêm Navigate
import {
    AppstoreOutlined,
    DashboardOutlined,
    FileTextOutlined,
...
const mainMenuItems: MenuItem[] = [
    ...
    { to: "/admin/courses", label: "Quản lý khóa học", icon: <AppstoreOutlined /> },
    { to: "/admin/exercises", label: "Quản lý bài tập", icon: <ReadOutlined /> },
    { to: "/admin/categories", label: "Quản lý thể loại", icon: <AppstoreOutlined /> },
    { to: "/admin/articles", label: "Quản lý bài viết", icon: <FileTextOutlined /> },
    { to: "/admin/users", label: "Quản lý người dùng", icon: <UsergroupAddOutlined /> },
    { to: "/admin/roles", label: "Quản lý nhóm quyền", icon: <IdcardOutlined /> },
     { to: "/admin/permissions", label: "Quản lý phân quyền", icon: <KeyOutlined /> }
];
...
const AdminLayout: React.FC = () => {
    ...
    const checkPermission = (path: string) => {
        ...
        if (path.includes("/exercises")) return permissions.includes("exercises_view");
        if (path.includes("/categories")) return permissions.includes("categories_view");
        if (path.includes("/articles")) return permissions.includes("articles_view");
        if (path.includes("/users")) return permissions.includes("view_user");
        if (path.includes("/settings")) return permissions.includes("settings_view");
        if (path.includes("/roles")) return permissions.includes("roles_view");
        if (path.includes("/permissions")) return permissions.includes("permissions_view");
    ...
    const filteredMainMenu = mainMenuItems.filter((item) => checkPermission(item.to));
    const filteredBottomMenu = bottomMenuItems.filter((item) => checkPermission(item.to));
    ...
    return (
        <div className="flex min-h-screen bg-brand-25 text-brand-900">
            {/* --- SIDEBAR --- */}
            <aside className={`flex min-h-screen flex-col border-r border-brand-100 bg-brand-50 transition-all duration-300 ${collapsed ? "w-[88px]" : "w-[280px]"}`}>
                
                <div className={`flex h-[76px] items-center ${collapsed ? "justify-center px-2" : "px-6"}`}>
                    {!collapsed ? (
                        ...
                        <NavLink to="/admin" className="flex h-10 w-10 items-center justify-center text-brand-600 text-lg font-extrabold">
                            <CodeOutlined />
                        </NavLink>
                    )}
                </div>
                ...
                <div className="flex-1 space-y-2 px-3 py-5">
                    {filteredMainMenu.map((item) => (
                        ...
                        >
                            ...
                            {!collapsed && <span className="text-base font-medium tracking-tight">{item.label}</span>}
                        </NavLink>
                    ))}
                </div>
                ...
                <div className="space-y-2 border-t border-brand-100 px-3 py-4">
                    ...
                    >
                        <span className="text-lg leading-none"><LogoutOutlined /></span>
                        {!collapsed && <span className="text-base font-medium tracking-tight">Logout</span>}
                    </button>
                </div>
                <button
            ...
            <div className="flex min-h-screen flex-1 flex-col">
                <header className="sticky top-0 z-20 flex  items-center justify-between border-b border-brand-100 shadow-sm bg-white px-6 py-3">
                    ...
                    <div className="relative w-full max-w-[320px]">
                        ...
                        <input type="text" placeholder="Tìm kiếm thể loại..." className="h-11 w-full rounded-2xl border border-brand-200 bg-white pl-11 pr-4 text-[15px] text-brand-900 outline-none transition placeholder:text-brand-300 focus:border-brand-400 focus:ring-2 focus:ring-brand-400/15" />
                    </div>
