...
export default function AuthForm({ type = "login" }: AuthFormProps) {
  ...
  const handleOk = async () => {
    ...
    try {
      ...
      showMessage("error", error.response?.data?.message || "Mã OTP không đúng");
    }
    setOTP("");
  };
  ...
  const handleSendEmail = async () => {
    if (!forgotEmail) return showMessage("error", "Vui lòng nhập email của bạn");
    try {
      ...
      showMessage("error", error.response?.data?.message || "Email không tồn tại trong hệ thống");
    }
  };
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      showMessage("error", error.response?.data?.message || "Mã OTP không đúng hoặc đã hết hạn");
    }
  };

  // BƯỚC 3: ĐẶT LẠI MẬT KHẨU MỚI
  const handleResetPassword = async () => {
    if (!newPassword || !confirmNewPassword) return showMessage("error", "Vui lòng điền mật khẩu mới");
    if (newPassword !== confirmNewPassword) return showMessage("error", "Mật khẩu xác nhận không khớp!");
    ...
    try {
      ...
      showMessage("success", "Đổi mật khẩu thành công! Vui lòng đăng nhập lại.");
  ...
  const handleResendOTP = async () => {
    try {
      ...
      showMessage("error", error.response?.data?.message || "Lỗi khi gửi lại OTP");
    }
  };
  ...
  const checkform = () => {
    if (tab === "register") {
      return Object.values(formRegisterData).every((value) => value.trim() !== "");
    } else {
  ...
  const onEmailChange = (value: string) => {
    ...
    const setFormData = tab === "register" ? setFormRegisterData : setFormLoginData;
  ...
  const onSubmit = async () => {
    ...
    if (!ischecked || errorEmail || errorPassword || errorConfirmPassword) {
      showMessage("error", `Vui lòng điền đầy đủ thông tin đăng ${tab === "register" ? "ký" : "nhập"}`);
      return;
    } 
    ...
    if (tab === "login") {
      try {
        ...
        showMessage("error", error.response?.data?.message || "Đăng nhập thất bại");
      }
    } else {
      try {
        ...
        showMessage("error", error.response?.data?.message || "Đăng ký thất bại");
      }
    }
  };
  ...
  return (
    <div className="w-full max-w-[390px] py-2">
      <div className="rounded-[26px] bg-white px-10 py-10 shadow-[0_8px_24px_rgba(0,0,0,0.05)]">
        ...
        <div className="mb-6 flex rounded-full bg-brand-25 p-1">
          ...
          <button
            ...
            className={`flex-1 rounded-full py-2.5 text-sm font-semibold transition ${
              tab === "register" ? "bg-white text-brand-700 shadow-sm" : "text-slate-500"
            }`}
          >
            Đăng ký
          </button>
        ...
        <div className="space-y-3">
          {tab === "register" && (
            <div>
              <label className="mb-1.5 block text-[13px] font-semibold text-slate-800">Họ và tên</label>
              <Input
                size="large"
          ...
          <div>
            ...
            <Input
              size="large"
              value={tab === "register" ? formRegisterData.email : formLoginData.email}
            ...
            {errorEmailTab === tab && errorEmail && (
              <p className="mt-1 text-[11px] text-red-500">Vui lòng nhập email hợp lệ.</p>
            )}
          </div>

          <div>
            <label className="mb-1.5 block text-[13px] font-semibold text-slate-800">Mật khẩu</label>
            <Input.Password
              size="large"
              value={tab === "register" ? formRegisterData.password : formLoginData.password}
            ...
            {errorPasswordTab === tab && errorPassword && (
              <p className="mt-1 text-[11px] text-red-500">Mật khẩu phải có ít nhất 6 ký tự.</p>
            )}
          </div>

          {tab === "register" && (
            <div>
              <label className="mb-1.5 block text-[13px] font-semibold text-slate-800">Xác nhận mật khẩu</label>
              <Input.Password
                size="large"
              ...
              {errorConfirmPassword && (
                <p className="mt-1 text-[11px] text-red-500">Mật khẩu xác nhận không khớp.</p>
              )}
            </div>
          )}
        </div>
        ...
        {tab === "login" && (
          <div className="mt-3 flex items-center justify-end gap-3">
            <button type="button" onClick={showForgotModal} className="text-[13px] font-semibold text-brand-700">
              Quên mật khẩu?
            </button>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-2.5">
          <button type="button" onClick={handleGoogleLogin} className="flex h-10 items-center justify-center gap-2 rounded-[12px] border border-brand-100 bg-white text-sm font-semibold text-slate-700 transition hover:bg-brand-25">
            <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" alt="Google" className="h-4 w-4" />
            Google
          </button>
          <button type="button" onClick={handleGithubLogin} className="flex h-10 items-center justify-center gap-2 rounded-[12px] border border-brand-100 bg-white text-sm font-semibold text-slate-700 transition hover:bg-brand-25">
        ...
        {tab === "register" && (
          <p className="mt-5 text-center text-[13px] text-slate-500">
            Đã có tài khoản?{" "}
            <button type="button" onClick={() => setTab("login")} className="font-semibold text-brand-700">
              Đăng nhập ngay
            </button>
          </p>
        )}
      ...
      <Modal
        ...
        okText="Xác nhận"
        cancelText="Hủy"
        okButtonProps={{ className: "!bg-brand-600 hover:!bg-brand-700 !border-none !rounded-xl" }}
        cancelButtonProps={{ className: "!rounded-xl !border-brand-200 hover:!text-brand-700 hover:!bg-brand-25" }}
      >
        <Input
      ...
      <Modal
        title={
          ...
          forgotStep === "otp" ? "Nhập mã xác nhận" : "Đặt lại mật khẩu"
        }
        open={isForgotModalOpen}
        onCancel={() => setIsForgotModalOpen(false)}
        onOk={
          forgotStep === "email" ? handleSendEmail : 
          forgotStep === "otp" ? handleVerifyOTP : handleResetPassword
        }
        okText={
          forgotStep === "email" ? "Gửi OTP" : 
          forgotStep === "otp" ? "Xác nhận OTP" : "Lưu mật khẩu mới"
        }
        cancelText="Hủy bỏ"
        okButtonProps={{ className: "!bg-brand-600 hover:!bg-brand-700 !border-none !rounded-xl" }}
        cancelButtonProps={{ className: "!rounded-xl !border-brand-200 hover:!text-brand-700 hover:!bg-brand-25" }}
      >
        <div className="pt-2">
            ...
            {forgotStep === "email" && (
              <>
                <p className="text-sm text-gray-500 mb-3">Vui lòng nhập email đã đăng ký, chúng tôi sẽ gửi mã xác nhận (OTP) gồm 5 chữ số cho bạn.</p>
                <Input
            ...
            {forgotStep === "otp" && (
              <div className="space-y-3">
                <p className="text-sm text-gray-500 mb-1">Mã OTP đã được gửi đến: <strong className="text-brand-600">{forgotEmail}</strong></p>
                <Input
                  ...
                  className="!h-11 !rounded-xl !bg-brand-25 !border-brand-100"
                />
                <p onClick={handleResendOTP} className="text-sm text-brand-600 hover:text-brand-700 font-medium cursor-pointer inline-block mt-2">
                  Gửi lại mã OTP
                </p>
              </div>
            )}
            ...
            {forgotStep === "password" && (
              <div className="space-y-3">
                <p className="text-sm text-green-600 mb-1 font-medium">Mã OTP hợp lệ! Hãy thiết lập mật khẩu mới.</p>
                <Input.Password
